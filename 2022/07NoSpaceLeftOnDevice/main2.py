...
def get_dir_size(dir: Dir, size_min: int):

    size_ret = 99999999999
    ...
    for dir_child in dir.dir_list:
        dir_size = get_dir_size(dir_child, size_min)
        if dir_size < size_ret:
            size_ret = dir_size
...
def main():
    ...
    with open("input.txt", "r", encoding="utf8") as file:
        ...
        fs_size_use = dir_tmp.get_size()
        fs_size_free = FS_SIZE_TOTAL - fs_size_use
        fs_size_to_free = UPDATE_SIZE - fs_size_free

        dir_size = get_dir_size(dir_tmp, fs_size_to_free)
        print(dir_size)
