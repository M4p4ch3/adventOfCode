...
class Path():
    ...
    def step(self, step_dir: str) -> bool:
        ...
        if (self.alt_grid[self.pos_cur.x][self.pos_cur.y] <=
                self.alt_grid[pos_new.x][pos_new.y] + 1):
            # At most one level lower
            ...
            # if self.alt_grid[pos_new.x][pos_new.y] > 0:
            #     # Keeping alt 0 does not count
            #     self.step_cnt += 1
    ...
    def check_reach_end(self) -> bool:
        """check end reached"""
        return self.alt_grid[self.pos_cur.x][self.pos_cur.y] == 0
...
def solve(data: str):
    ...
    #         if alt_grid[grid_x][grid_y] == 0:
    #             pos_start = Coord(grid_x, grid_y)
